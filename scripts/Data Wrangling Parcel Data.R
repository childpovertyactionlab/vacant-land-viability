...
## load data 
selected_AI <- read_csv("Data/ACCOUNT_INFO.CSV") %>% select(ACCOUNT_NUM, DIVISION_CD, OWNER_NAME1,
                                                                 STREET_NUM, FULL_STREET_NAME, 
                                                                 PROPERTY_CITY, PROPERTY_ZIPCODE, GIS_PARCEL_ID) %>%
  mutate(PROPERTY_ZIPCODE = ifelse(nchar(PROPERTY_ZIPCODE) >= 5, str_sub(PROPERTY_ZIPCODE, 1, 5), PROPERTY_ZIPCODE))
selected_AAY <- read_csv("Data/ACCOUNT_APPRL_YEAR.CSV") %>% select(ACCOUNT_NUM, SPTD_CODE)
selected_par <- st_read('Data/PARCEL/PARCEL.shp') %>% 
...
selected_tract <- tracts(state = "TX", county = "Dallas")%>%
...
## subset by industrial zoning only

indus_df <- joined_df %>%
  filter(SPTD_CODE %in% c("C13", "F20", "G10", "G30", "J10", "J20", "J30", "J40", "J51", 
                          "J52", "J60", "J70", "L20"))

## export dataset to working directory

st_write(indus_df, "Data/Processed Data/Industrial DCAD Data.geojson", delete_dsn = TRUE)
...
# manually edit a few owners that fit both FBO and public conditions

cleaned_accounts <- cleaned_accounts %>%
  mutate(PUBLIC_ACCOUNT = case_when(
    OWNER_NAME1 == "CITY OF HOPE PROJECT" ~ FALSE,
    TRUE ~ TRUE
  ))
...
# Join parcels with final_df
vacant_sf <- inner_join(selected_par, final_df, by = "GIS_PARCEL_ID")

# Join Census Tracts

selected_tract <- st_transform(selected_tract, crs = st_crs(vacant_sf))

vacant_sf <- st_join(vacant_sf, selected_tract, join = st_intersects)

# Join city council boundaries
selected_council <- st_transform(selected_council, crs = st_crs(vacant_sf))

vacant_sf <- st_join(vacant_sf, selected_council, join = st_intersects)

# Calculate the area in square meters for multipolygons
vacant_sf <- vacant_sf %>%
  mutate(area = st_area(geometry)) #%>%
  #mutate(area_sqft = as.numeric(area_sqm) * 10.7639)

# Convert multipolygons to points )
  vacant_points <- vacant_sf %>%
  st_point_on_surface()

# Transform back to the original CRS 
vacant_points <- st_transform(vacant_points, crs = 4269)
